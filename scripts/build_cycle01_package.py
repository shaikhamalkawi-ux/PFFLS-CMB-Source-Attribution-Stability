#!/usr/bin/env python3
"""Build a deterministic Cycle 01 review ZIP and SHA-256 manifest."""
...
PACKAGE_NAME = "PFFLS_CODEX_CYCLE01_RETURN_20260920"
...
def included_files(root: Path) -> list[Path]:
    candidates = [
        ...
        root / "tasks" / "CODEX_CYCLE_01.md",
        root / "requirements-cycle01.txt",
        *sorted((root / "outputs" / "cycle01").glob("*")),
        root / "scripts" / "validate_cycle01_evidence.py",
        root / "scripts" / "fetch_cycle01_sources.py",
        root / "scripts" / "build_cycle01_package.py",
        root / "tests" / "test_cycle01_evidence.py",
    ]
    manifest = root / "outputs" / "cycle01" / "artifact_manifest_sha256.csv"
    return [path for path in candidates if path.is_file() and path != manifest]


def write_manifest(root: Path, paths: list[Path]) -> Path:
    target = root / "outputs" / "cycle01" / "artifact_manifest_sha256.csv"
