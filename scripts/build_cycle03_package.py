#!/usr/bin/env python3
"""Build the deterministic Cycle 03 review package and SHA-256 sidecar."""
...
PACKAGE_NAME = "PFFLS_CODEX_CYCLE03_REMAINING_EVIDENCE_RETURN"
...
def included_files(root: Path) -> list[Path]:
    manifest = root / "outputs" / "cycle03" / "artifact_manifest_sha256.csv"
    candidates = [
        root / "tasks" / "CODEX_CYCLE_03_REMAINING_EVIDENCE.md",
        *sorted((root / "outputs" / "cycle03").glob("*")),
        root / "scripts" / "audit_cycle03_evidence.py",
        root / "scripts" / "build_cycle03_package.py",
        root / "tests" / "test_cycle03_evidence.py",
    ]
    return [path for path in candidates if path.is_file() and path != manifest]


def write_manifest(root: Path, paths: list[Path]) -> Path:
    target = root / "outputs" / "cycle03" / "artifact_manifest_sha256.csv"
